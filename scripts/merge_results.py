...
Reads from /tmp/artifacts/ds-results-*/results.json
...
def send_summary_email(new_count: int, total: int, timestamp: str):
    ...
    subject  = f"Dermatology & Skin Science Research Digest — {run_date} | {new_count} {'Study' if new_count == 1 else 'Studies'}"
    html = f"""<!DOCTYPE html>
<html>
