...
Your readers pitch to publications like Allure, Byrdie, Vogue Health, Self, Women's Health Magazine, The Derm Review, and science-forward beauty and health outlets.
...
{{
  ...
  "groundbreaking": "one or more of: Counterintuitive finding / Overturns prior research / First-in-class human study / Major skincare or skin health implication",
  "media_coverage": "{media_note}",
  "summary": "2-4 sentences: what researchers did, who or what was studied (N=X, skin type, condition, or demographic if relevant), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for people interested in skin health, skincare, or dermatological conditions. Do NOT imply clinical action.",
  "caveats": "comma-separated flags: small sample (N<100), observational design, single-center, self-reported outcomes, short follow-up, industry funding [name], preprint, secondary analysis, limited skin tone diversity in sample, animal or cell study — or 'None identified'",
  "fact_check_note": "Describe any corrections made vs what the abstract actually says, or empty string",
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for skincare or dermatology journalism",
  "pitch_angles": [
    {{
      "publication_type": "e.g. Allure / Byrdie / Vogue Health / Self / Women's Health / The Derm Review / General health",
      ...
      "pitch_angle": "2-3 sentence commissioning pitch: what happened, why it matters for skin health or skincare, product/routine or clinical significance",
...
Rules for content:
- Verify sample size (N), study design, and direction of effect against the abstract
- For biochemistry: only include if the finding has a direct skin relevance — collagen synthesis, UV damage mechanisms, barrier function, pigmentation, ingredient absorption; exclude purely non-skin biochemistry
- For allergy and immunology: focus on skin-relevant conditions (eczema/atopic dermatitis, contact dermatitis, urticaria, food allergy with skin manifestations, immune-mediated skin diseases)
...
relevance_score rubric (1–10): start at 5, then adjust:
  +2 counterintuitive or overturns widely held belief about skincare or skin conditions
  +2 human subjects (N≥100) with clinically or cosmetically relevant outcome
  +1 clear practical hook (a common ingredient, routine behavior, or widespread condition)
  +1 clean design (RCT, well-controlled clinical trial)
  +1 addresses a high-prevalence condition (acne, eczema, psoriasis, rosacea, photoaging)
  −1 per major caveat
  −1 homogeneous skin tone sample with findings that may not generalize
  −2 animal or cell study only
  Topic fit bonus: retinoids and skin aging, sunscreen and UV protection, microbiome of the skin, eczema and atopic dermatitis, acne mechanisms, melanoma and skin cancer, ingredient efficacy (niacinamide, hyaluronic acid, peptides), skin barrier function, rosacea, psoriasis biologics, cosmetic procedure safety score higher
...
{studies_block}"""
    ...
    try:
        ...
        return [{"pmid": s["pmid"], "headline": s["title"], "journal": s["journal"],
                 ...
                 "groundbreaking": "Major skincare or skin health implication", "media_coverage": media_note,
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    subject = f"Dermatology & Skin Science Research Digest — {run_date} | {study_count} {'Study' if study_count == 1 else 'Studies'}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#1a3a2e;">Dermatology &amp; Skin Science Research Digest</h2>
...
<p style="font-size:0.85em;color:#888;margin-top:2em;">
  Dermatology &amp; Skin Science Research Digest · PubMed + Claude + SERPAPI
</p>
</body>
</html>"""
    try:
...
def main():
    ...
    print("=" * 60)
    print("Dermatology & Skin Science Research Digest")
